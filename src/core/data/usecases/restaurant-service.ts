...
export class RestaurantService implements RestaurantUseCase {
  ...
  async load(): Promise<Array<Restaurant>> {
    try {
      const response = await this.httpClientService.send({
        method: 'GET',
        url: this.host.URL.RESTAURANT,
        params: {city: 'Toronto'},
      });
      const parseJson = response.body.restaurants.map(
        (restaurant: any): Restaurant => {
          ...
          return {
            ...
            mobileReserveUrl: mobile_reserve_url,
            imageUrl: image_url,
          };
        }
      );
