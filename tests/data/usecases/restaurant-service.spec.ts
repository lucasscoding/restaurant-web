...
import { RestaurantUseCase } from '../../../src/domain/usecases/restaurant-usecase'
import { hostConfig } from '../mocks/configs/env-config'
import { HttpClientStub } from '../mocks/stubs/http-client-stub'
import { RestaurantService } from '../../../src/data/usecases/restaurant-service'
...
describe('RestaurantService', () => {
  ...
  it('shoud load and return five itens in array', async () => {
    ...
    jest.spyOn(httpClientStub, 'send').mockResolvedValueOnce({ statusCode: 200, body: { restaurants: mock }})
  ...
  it('should re throws the error when load restaurants', async () => {
    jest.spyOn(httpClientStub, 'send').mockImplementationOnce(() => { throw new Error() })
    const result = restaurantService.load()
    await expect(result).rejects.toThrow()
  })

  it('should load stats of restaurants', async () => {
    jest.spyOn(httpClientStub, 'send').mockResolvedValueOnce({ statusCode: 200, body: { countries: 10, cities: 20, restaurants: 30 }})
